...
def clientdisconnect(Client, Content):
    ...
    nb_clients -=1
@server.event
def pwd(Client,Content):
    global usrlist,pwdlist
    if Content[0] == "register":
        pass
    if Content[0] == "login":
        if Content[1] in usrlist and Content[2] in pwdlist:
            Client.send_message("login",True)
        else:
            Client.send_message("login",False)
